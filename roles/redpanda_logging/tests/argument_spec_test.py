...
def test_valid_inputs_pass():
    status, msg = run_playbook({'redpanda_logging_enabled': False})
    assert status == 'successful', f"valid inputs must validate: {msg}"


@pytest.mark.parametrize("bad,var", [({'redpanda_logging_enabled': 'banana'}, 'redpanda_logging_enabled'), ({'redpanda_logging_enabled': False, 'redpanda_logging_systemd_max_level': 'verbose'}, 'redpanda_logging_systemd_max_level')])
